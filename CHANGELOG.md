...
## [Unreleased]
### Added
- **Consistent header toolbar on every page.** Language, theme (sun/moon), Demo and Support now live in one shared toolbar (`assets/toolbar.css`) on the landing, the app and the bib generator — the same controls are always one tap away, with **Open app** kept as the prominent primary action. The bib generator previously had neither the language picker nor these controls; it now does (and its donation button opens the same explainer modal). The bib generator's own form text is still English for now.
