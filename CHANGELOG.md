...
## [Unreleased]
### Added
- **Multi-language (stage 1).** Crono can now run in **English, Romanian, Spanish, German, French, Japanese, Chinese and Hindi**. A language picker in the app header switches instantly and remembers the choice (`crono.lang`); with no choice it follows the device language. Built zero-dependency and offline-first (`assets/i18n.js`), with system fonts covering Japanese/Chinese/Hindi. This stage covers the app's main UI; the landing page, the app's dynamic messages (toasts/dialogs/table built in JS) and exports are being migrated next. Legal text stays English by design.
