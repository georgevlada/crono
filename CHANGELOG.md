...
- OG image generator (`tools/make-og.cjs`): sharper anti-aliasing (4× supersample), fixed an inverted "U" glyph (it read as "YOWR"), condensed the wordmark and lifted sub-line contrast. Social-share art still being finalised.
...
### Added
- **Live preview in the bib-number generator.** The "Print bib numbers" modal now shows a WYSIWYG preview of a sample bib that updates as you type the event name, date, range and choose a logo — so you see the layout before printing. The theme picker is now four colour **swatches** (orange / lime / blue / black & white) instead of a dropdown. The four theme colours are single-sourced as CSS tokens shared by the swatches, the preview and the print sheet.
