...
## [Unreleased]
### Added
- **Multi-language.** Crono can now run in **English, Romanian, Spanish, German, French, Japanese, Chinese and Hindi**. A language picker in the app header switches instantly and remembers the choice (`crono.lang`); with no choice it follows the device language. Built zero-dependency and offline-first (`assets/i18n.js`), with system fonts covering Japanese/Chinese/Hindi. The **whole app** is now translated — including dynamic messages: ranking tabs and categories, toasts, confirm dialogs, the participants/edit/demo modals and the empty states (they re-render live when you switch language). The landing page is being migrated next; CSV/PDF export headers stay English for data portability, and legal text stays English by design.
